...
(3,7,11) is 147 at (2,3,2): 2^3 + 3^7 - 2^11 = 147 (NOT 277 at (13,2,2),
which is rank 4). Use near_miss_package.py / audit_corrected_scan.py.
Retained unmodified for the record.

Signature (3,7,11) probe (attempt-20 / cycle 18): A^3 + B^7 = C^11.

Tests the prediction from attempt-19 (counting heuristic): the min
non-degenerate COPRIME near-miss gap should grow monotonically as chi =
1/p+1/q+1/r-1 grows more negative.
  (3,5,7):  chi = -34/105 ~ -0.324  -> min gap 29
  (3,5,11): chi = -62/165 ~ -0.376  -> min gap 77
  (3,7,11): chi = -100/231 ~ -0.433 -> PREDICTION: min gap > 77

Universal degenerate gap-1 families for (p,q,r)=(3,7,11):
  t^33 + 1 : A=t^11, B=1, C=t^3   (A^3=t^33, C^11=t^33, gap +1)  [lcm(3,11)=33]
  t^77 + 1 : A=1, B=t^11, C=t^7   (B^7=t^77, C^11=t^77, gap +1) [lcm(7,11)=77]

Run:  python search_3711.py
"""
import math
...
A_MAX = 6000
B_MAX = 600
C_MAX = 40   # C^11 grows fast; A_MAX^3 ~ 2.16e14 => C<=~20 A-dominated, ~40 with B^7
...
print("=== exact solutions A^3+B^7=C^11 over A<=%d B<=%d C<=%d ===" % (A_MAX, B_MAX, C_MAX))
print("total exact:", len(exact), "| coprime:", len(exact_coprime))
for t in exact[:30]:
    print("   ", t, "gcd=%d" % gcd3(*t))

# ---- gap-1 near-misses |A^3+B^7-C^11|==1 --------------------------------
...
print()
print("=== gap-1 near-misses |A^3+B^7-C^11|==1 ===")
...
print()
print("chi values: (3,5,7)=%.4f (gap 29) | (3,5,11)=%.4f (gap 77) | (3,7,11)=%.4f"
      % (-34/105, -62/165, -100/231))