...
[ATTACK mss-k34-refine4] 2026-09-02 ~12:35 | magic-square-of-squares | depth-parity sieve (new kill layer)
- NEW LAYER: kernel primes never killed in the old sieve (Lemma 2: X(nG) = 0 mod q is a QR); their DEPTH PARITY is new kill info. Kill condition: d=ord_q(G) | n, q does not divide n, b_d + v_q(d) odd. Notes sec 2h filed.
- Parity table (parityA/parityB.json): 25,995 good primes <= 3e5 (A), 17,982 <= 2e5 (B); 99.99 percent odd base depth (killers).
- SIEVE RESULTS (k <= 2e5, validated vs direct per-k brute check EXACTLY on all 4 B classes, 2,945 k0 spot-checks): kills 64.0/42.1 percent of k (A c=+/-2, c=+/-1|M/2-1), 56.1/78.0 percent (B). No class collapses; survivors ~36-58 percent.
- Kill-correlation finding: per-prime density models overestimate kill power up to 25x (primes sharing an order impose identical conditions); distinct-(D,k0) model matches A c=+/-1 (0.554 vs 0.579) but still overkills B c=2 (0.105 vs 0.780) — nested progressions cluster. Observed densities = ground truth.
- Valid primes NEVER kill nonzero cosets (gcd(d,M)|c and d|M force d|c, impossible d>=4) — consistent with class-0 constraint theorem. Honest: layer does not collapse the cosets; gap remains as in sec 2g. K34 OPEN.
